...
model = YOLO("C:/Users/ADMIN/Desktop/New folder/airsim setup-20240224T165409Z-001/airsim setup/droneLanding-main/droneLanding-main/best.pt",verbose=False)
...
class CustomEnv(gym.Env):
    def __init__(self, vehicle_name="", num_history=3):
        ...
        self.observation_space = spaces.Box(low=0, high=1, shape=(self.num_history,8), dtype=np.float32)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.drone_pose.position.x_val = random.randint(-2, 2)
        self.drone_pose.position.y_val = random.randint(-2, 2)
    ...
    def step(self, action):
        ...
        self.client.moveByVelocityBodyFrameAsync(vx, vy, vz, 0.5,vehicle_name = self.vehicle_name)
        '''
        #print(action)
    ...
    def _get_bounding_box_info(self):     
        ...
        if len(results[0].boxes) > 0:
            ...
            if box.id == None:
                ...
                tracker_id = np.float32(box.id.item())
    ...
    def _compute_reward(self):
        ...
        if collide_info.has_collided:
            ...
            if "BP_Helipad" in collide_info.object_name:
                reward = +50
                terminations = True
                truncations = False
        ...
        drone_position = drone_kinematics.position.to_numpy_array()
        drone_velocity = drone_kinematics.linear_velocity.to_numpy_array()
        
        vel_mag = np.linalg.norm(drone_velocity)
        #distance_to_origin = np.linalg.norm(drone_position)
        distance_to_center = np.sqrt((cX - 0.5)**2 + (cY - 0.5)**2)
        
        # Set the steepness factor
        alpha = 0.5
        max_velocity = np.sqrt(self.max_speed**2 * 3)
        # Calculate exponential reward
        reward += 2*np.exp(-alpha * distance_to_center)
        reward +=  1 - vel_mag/max_velocity
        reward += 1 / (1 + 0.5 - box_area)
